...
#Separa os dados previsores de forma proporcional em treinamento e teste, de forma aleatoria  
    for indice_treinamento, indice_teste in kfold.split(previsores, np.zeros(shape = (previsores.shape[0], 1))):
        ...
        classificador_svm = SVC(random_state = 1, C = 2.0)
        classificador_mlp = MLPClassifier(hidden_layer_sizes = (200), max_iter = 3000, tol = 0.0000010, verbose = True)
        classificador_forest = RandomForestClassifier(n_estimators = 40)
